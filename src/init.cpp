...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "odex.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "odexd.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the ODEX and zODEX money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 20149, 21149));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in ODEX/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in ODEX/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, odex, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in ODEX/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all ODEX specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-backupzodex=<n>", strprintf(_("Enable automatic wallet backups triggered after each zODEX minting (0-1, default: %u)"), 1));
    strUsage += HelpMessageOpt("-resetspentzerocoin", strprintf(_("*You can lose zODEX if you run this before the spend is confirmed on the blockchain.*  Reset current zODEX spends to match those on the blockchain.%s"), ""));
    strUsage += HelpMessageOpt("-resetmintzerocoin", strprintf(_("*You can lose zODEX if you run this before the mint is confirmed on the blockchain.*  Reset current zODEX mints to match those on the blockchain.%s"), ""));

//    strUsage += "  -anonymizeodexamount=<n>     " + strprintf(_("Keep N ODEX anonymized (default: %u)"), 0) + "\n";
//    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to Obfuscation by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += HelpMessageOpt("-rpcport=<port>", strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 20150, 21150));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
	   FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Odex Core Developers"), COPYRIGHT_YEAR)) + "\n" +
	   "\n" +
...
/** Sanity checks
 *  Ensure that Odex is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Odex Core is shutting down."));
...
#endif
    // Make sure only a single Odex process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Odex Core is probably already running."), strDataDir));
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                //Odex specific: zerocoin and spork DB's
                ...
                // Odex: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    ...
                    if (nZerocoinStartHeight != 0) {
                        if (chainActive.Height() > nZerocoinStartHeight) {
                            RecalculateZODEXMinted();
                            RecalculateZODEXSpent();
                        }
                    }
                    RecalculateODEXSupply(1);
                }
                ...
                // Odex: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Odex Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart Odex Core to complete") << "\n";
        ...
        bool fEnableZODEXBackups = GetBoolArg("-backupzodex", true);
        pwalletMain->setZODEXAutoBackups(fEnableZODEXBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeOdexAmount = GetArg("-anonymizeodexamount", 0);
//    if (nAnonymizeOdexAmount > 999999) nAnonymizeOdexAmount = 999999;
//    if (nAnonymizeOdexAmount < 2) nAnonymizeOdexAmount = 2;
    ...
    LogPrintf("Anonymize Odex Amount %d\n", nAnonymizeOdexAmount);
    ...
    /* Denominations
       ...
       For example:
       1ODEX+1000 == (.1ODEX+100)*10
       10ODEX+10000 == (1ODEX+1000)*10
    */
