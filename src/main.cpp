...
#if defined(NDEBUG)
#error "Odex cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uODEX) are considered zero fee (for relaying and mining)
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zODEX spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zODEX spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zODEX spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int blockHeight)
{
    if (blockHeight == 0){
        nSubsidy = 300000 * COIN;
    } else if (blockHeight <  100 && blockHeight >= 1) {
        nSubsidy = 1;
    } else if (blockHeight <  500 && blockHeight >= 100) {
        nSubsidy = 1.25;
    } else if (blockHeight <  10000 && blockHeight >= 500) {
        nSubsidy = 1.5;
    } else if (blockHeight <  265000 && blockHeight >= 10000) {
        nSubsidy = 5;
    } else if (blockHeight <  530000 && blockHeight >= 265000) {
        nSubsidy = 4.5;
    } else if (blockHeight <  1060000 && blockHeight >= 530000) {
        nSubsidy = 4;
    } else if (blockHeight <  1590000 && blockHeight >= 1060000) {
        nSubsidy = 3.5;
    } else if (blockHeight <  2120000 && blockHeight >= 1590000) {
        nSubsidy = 3;
    } else if (blockHeight <  2650000 && blockHeight >= 2120000) {
        nSubsidy = 2;
    } else if (blockHeight <  3180000 && blockHeight >= 2650000) {
        nSubsidy = 1.4;
    } else if (blockHeight <  3710000 && blockHeight >= 3180000) {
        nSubsidy = 1.05;
    } else if (blockHeight >= 3710000) {
        nSubsidy = 0.75;
    }
    return nSubsidy;
}
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from Odex
...
{
    RenameThread("odex-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZODEXMinted()
{
    int nZerocoinStartHeight = GetZerocoinStartHeight();
...
void RecalculateZODEXSpent()
{
    ...
    while (true) {
        ...
        //Remove spends from zODEX supply
...
bool RecalculateODEXSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zODEX with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zODEXSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert ODEX to zODEX
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zODEX spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zODEX serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zODEX mints and %d zODEX spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // Odex: We use certain sporks during IBD, so check to see if they are
